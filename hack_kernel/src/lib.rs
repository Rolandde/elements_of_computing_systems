...
//! The 32K [memory][architecture::DataMemory] is much too slow to be used in the [Computer]. It takes more than 10 seconds to scan through the screen with the release build. This is because a real hardware chip works in parallel, but the software emulated hardware does not. To read or write to the emulated memory, each address has to be visited one at a time. To stress this, specifying a single address still leads to every address being visited. This is an unavoidable outcome of following the rules of the game: building the emulated computer hardware from a single chip.
//!
